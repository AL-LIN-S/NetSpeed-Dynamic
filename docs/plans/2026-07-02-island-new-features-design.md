...
## 0. 实现状态总览（最终落地，2026-07-03 更新）

原计划 5 个功能，实际落地经多轮验证后调整如下：

| 功能 | 状态 | 说明 |
|------|------|------|
| 番茄钟 | ✅ 已实现 | 四阶段循环，岛内右侧显示倒计时（非外挂角标），控制台可开始/暂停/跳过/重置 |
| 久坐/喝水提醒 | ✅ 已实现 | `GetLastInputInfo` 真实闲置检测，离开自动重置；与番茄钟共用全岛弹出提醒通道 |
| QQ 消息提醒 | ✅ 已实现 | 复用现有系统 Toast 监听，前提是 QQ 开启系统通知 |
| 微信消息提醒 | ❌ 放弃 | 新版 Weixin 4.x **不发系统 Toast**，标题也不带未读数；窗口标题/系统通知两条路都断，仅剩重型注入方案（不划算） |
| 天气 | ⏭ 跳过 | 需注册和风 API key，未做 |
| 快捷启动器 | ❌ 移除 | 已实现后按反馈不需要，`git revert` 回滚 |

**附带修复**（不在原 5 项内）：移除假 GPU 显示、延迟探测多端点化、通知监听权限校验、移除微信通知硬过滤（让微信 Toast 在未来版本若发送时即可用）。

**关键架构事实**：消息提醒走的是 `QQ/微信 → Windows 通知中心 → UserNotificationListener → 灵动岛` 这条链路。能否收到取决于**应用是否真的发系统 Toast**，与代码无关。
